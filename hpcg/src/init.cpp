...
//strcpy(params.yamlFileName, optarg);
  ...
  for (i = 1; i <= argc && argv[i]; ++i)
  {
      ...
      {
          /* if invalid yamFileName, use default filename (obtained by concatenating the miniAppName + miniAppVersion + ".yaml" strings) */
          if (sscanf(argv[i]+strlen("--yaml="), "%s", params.yamlFileName) != 1) params.yamlFileName[0] = '\0';
      }
  }
